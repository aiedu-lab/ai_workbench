...
STUB (mock, pending ai_workbench's real codebase): ai_workbench has
...
commands here once ai_workbench has bazel oci_image targets to
validate.
...
def skill_build_and_test(repo_root: Path) -> None:
  ...
  add them once ai_workbench has bazel oci_image targets to validate.
  """
  commands = [
    ["bazel", "build", "//..."],
    ["bazel", "test", "//..."],
