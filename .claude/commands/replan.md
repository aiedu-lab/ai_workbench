...
- Flips every `[ ] Status` → `[x] Status` in the Phase N+1
  block of `sdw/plan.md` (all steps should already be appended
  from Step 3 — only the status lines change here).
- Commits all changed files and tags
  `vN+1.K-<brief-summary>-step-completed`.

Note: `sdw/prompt_history.md` `[ ] Status` was already changed
to `[x] Status` and committed right after plan approval (Step 3).
...
1. Change `[ ] Status` → `[x] Status` on the line after
   `## <title>` in `sdw/prompt_history.md`.
2. Append the full Phase N+1 block to `sdw/plan.md`. Each step
   in the block must follow the plan-step template exactly —
   including its `[ ] Status` line — using a condensed
   (one-paragraph) body per field rather than multi-line.
3. Commit both files together:
   ```bash
   git add sdw/plan.md sdw/prompt_history.md
   git commit -m "chore: Phase N+1 - append plan + mark prompt complete"
   ```

This ensures `sdw/plan.md` always reflects the approved plan
and `sdw/prompt_history.md` is committed with the prompt that
drove the plan, regardless of how many steps are executed.
...
3. Flip the step's `[ ] Status` → `[x] Status` in `sdw/plan.md`.
...
1. Confirms all `[ ] Status` lines in Phase N+1 of `sdw/plan.md`
   are already `[x] Status` (flipped per-step during execution).
...
grep -A1 "### Step N+1\." sdw/plan.md | grep "\[ \] Status"
# → 0 matches

# prompt_history.md section already marked complete
grep -A1 "^## <title>" sdw/prompt_history.md | grep "\[x\] Status"
# → 1 match
