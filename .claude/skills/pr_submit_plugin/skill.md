...
description: >
  ...
  build/test step is mocked pending ai_workbench's real codebase (no
...
**STUB (mock, pending ai_workbench's real codebase):** the build+test
skill step below only runs `bazel build //...` / `bazel test //...`
— no container-test commands, since ai_workbench has no bazel
oci_image targets yet. Extend `pr_submit_plugin.py`'s
