...
CONTEXT: `tools/claude/cli.md` has an uncommitted draft edit already replacing "Plugin Installs" Step 6 (`claude plugin update` → two `claude plugin marketplace update ...` lines) and an uncommitted edit retitling the "🔐 Security" heading to "🔐 Security: OAUth Token an dAPI Keys" (typos) plus a new "Reference section above `Subscription / OAuth Token Mode`" note. "Install the VSCode Extension" (lines 64-91) has a verbose "Once installed:" paragraph with 4 usage bullets before the "keep the sidebar open" `settings.json` tip.
