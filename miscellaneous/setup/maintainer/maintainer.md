...
```bash
gh auth status
gh api repos/aiedu-lab/ai_workbench --jq '.permissions.maintain'
```
...
```bash
gh pr list --repo aiedu-lab/ai_workbench
```

```bash
gh pr view <number> --repo aiedu-lab/ai_workbench
```
...
```bash
gh pr edit <number> --repo aiedu-lab/ai_workbench \
  --title "New title" \
  --base main
```
...
```bash
gh pr close <number> --repo aiedu-lab/ai_workbench \
...
```bash
gh pr list --repo aiedu-lab/ai_workbench --state all --limit 5
```
