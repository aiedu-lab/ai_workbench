...
```bash
gh auth status
gh api repos/aiedu-lab/ai_workbench --jq '.permissions.admin'
```
...
Expected: path-specific rules for `plans/`, `projects/`,
`sessions/`, `learnings/`, `tools/`, and a fallback `* @asarcar`.
...
These settings are already live on this repo. To re-apply or verify
via `gh` (the nested `required_pull_request_reviews` object must be
sent as raw JSON via `--input` — `gh api`'s dotted `-f`/`-F`
flattening fails with `"required_pull_request_reviews" wasn't
supplied` on `gh` 2.79.0):

```bash
cat <<'EOF' | gh api -X PUT \
  repos/aiedu-lab/ai_workbench/branches/main/protection \
...
```bash
gh api repos/aiedu-lab/ai_workbench/branches/main/protection \
  --jq '{
...
For a full explanation of how the two credential types work,
when each applies, and how OAuth token auth differs from API key
auth, see:
- [`miscellaneous/tools/claude/cli.md`](
  ../tools/claude/cli.md) — switching auth modes, convenience
  functions, validation commands
- [`miscellaneous/experimental/docs/auth.md`](
  ../experimental/docs/auth.md) — detailed OAuth 2.0 flow, WSL
  workaround, subscription billing model
...
browser can open (not WSL without the workaround in `auth.md`):
...
**`gh` CLI** (value read interactively — never put the token inline
on the command line where it would appear in shell history):

```bash
gh secret set CLAUDE_CODE_OAUTH_TOKEN \
  --repo aiedu-lab/ai_workbench

gh secret set ANTHROPIC_API_KEY \
  --repo aiedu-lab/ai_workbench
```

`gh secret set` prompts interactively for the value, or pipe it:
`echo "$TOKEN" | gh secret set ... --body -`.

Validation (names only — values are never shown):

```bash
gh secret list --repo aiedu-lab/ai_workbench
```
...
```bash
gh api repos/aiedu-lab/ai_workbench/collaborators/<username>/permission \
  --jq '.permission'
```
...
```bash
gh api -X PUT repos/aiedu-lab/ai_workbench/collaborators/<username> \
  -f permission=push
```
