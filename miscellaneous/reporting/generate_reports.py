...
# Matches an Agenda row: `| [**Title**](sessions/slug.md) | Desc | ... |`
AGENDA_ROW_RE = re.compile(
  r"^\|\s*\[\*\*(?P<title>[^\]]+)\*\*\]"
  r"\(sessions/(?P<slug>[\w-]+)\.md\)\s*\|\s*(?P<desc>[^|]+)\|"
)
...
def parse_agenda() -> dict[str, tuple[str, str]]:
  ...
  text stays single-sourced instead of duplicated here. Only rows
  whose slug has a matching `projects/<slug>/` directory are kept -
  concept-only sessions have no project to submit a solution under.
  """
  ...
  for line in README_PATH.read_text().splitlines():
    ...
    if not (PROJECTS_DIR / slug).is_dir():
      continue
    topics[slug] = (match.group("title").strip(), match.group("desc").strip())
  return topics
...
def collect_completions(
  ...
  for slug, (title, _desc) in topics.items():
...
def resolve_full_name(userid: str) -> str:
  ...
  request = urllib.request.Request(
    ...
    headers={"User-Agent": "ai-workbench-report"},
  )
  try:
...
def student_table(
  ...
  for slug, (title, desc) in topics.items():
    link = f"[{title}](../../../sessions/{slug}.md)"
    exercises = completions[slug]
    if not exercises:
    ...
    for exercise in sorted(exercises):
      ...
      lines.append(f"| {link} | {exercise} | {desc} | {mark} |")
  return "\n".join(lines)
