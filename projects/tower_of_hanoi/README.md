...
This exercise illustrates **Human Directed Development (HDD)** —
a workflow where a human defines the architecture, test structure,
and acceptance criteria while delegating repetitive implementation
to an AI coding agent.

| Phase | You provide | AI produces |
|---|---|---|
| 1. Architecture | `solution/prompts/toh_problem_prompt.md` | Scaffold classes + CLI |
| 2. Test structure | `solution/prompts/toh_define_tests_prompt.md` | Test stubs (no assertions) |
| 3. Test assertions | `solution/prompts/toh_complete_tests_prompt.md` | Full test suite |
| 4. Implementation | `solution/prompts/toh_complete_solution_prompt.md` | Working classes |
...
│       └── test_integration.py
└── solution/               # Reference material (do not copy)
    ├── prompts/            # Sample HDD prompt files
    │   ├── toh_problem_prompt.md
    │   ├── toh_define_tests_prompt.md
    │   ├── toh_complete_tests_prompt.md
    │   └── toh_complete_solution_prompt.md
    └── src/                # Sample solution generated by AI
        └── ...             # (mirrors src/ layout)
```
...
| Phase | Prompt file |
|---|---|
| 1. Define architecture + scaffold | `solution/prompts/toh_problem_prompt.md` |
| 2. Generate test structure (stubs) | `solution/prompts/toh_define_tests_prompt.md` |
| 3. Fill in test assertions | `solution/prompts/toh_complete_tests_prompt.md` |
| 4. Implement skeleton classes | `solution/prompts/toh_complete_solution_prompt.md` |

```bash
# Run phase 1 (or substitute any prompt file for other phases).
# Claude will ask for permission before each write action.
claude -p "$(cat solution/prompts/toh_problem_prompt.md)" \
