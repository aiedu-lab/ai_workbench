...
def empty_tower() -> Tower:
  # num_discs=3 is the game capacity for AsciiRenderer column
  # sizing, not the current disc count on this tower.
...
class TestTowerPush:
  ...
  def test_push_larger_disc_on_smaller_raises(self, empty_tower):
    """Placing a larger disc on a smaller one raises ValueError."""
...
class TestTowerAsSizeList:
  def test_full_tower_size_list(self, full_tower):
    """as_size_list() returns disc sizes ordered bottom→top."""
    assert full_tower.as_size_list() == [3, 2, 1]

  def test_empty_tower_size_list(self, empty_tower):
    """as_size_list() on an empty tower returns an empty list."""
