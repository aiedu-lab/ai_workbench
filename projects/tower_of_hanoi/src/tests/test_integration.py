...
def run_puzzle(tmp_path, num_discs: int) -> tuple:
    ...
    orch = Orchestrator(num_discs=num_discs, step_by_step=False, step_file=step_file)
    orch.run()
...
class TestEndToEndLarger:
    ...
    def test_no_disc_ordering_violation_throughout(self, tmp_path):
        """After every move, each tower must have discs in strictly descending order."""
        ...
        move = Move(towers=towers, num_discs=num_discs, step_by_step=False, step_writer=writer)
