...
def move_3(tmp_step_file):
    ...
    return Move(towers=towers, num_discs=3, step_by_step=False, step_writer=writer), towers, writer
...
class TestMoveNext:
    ...
    def test_next_advances_board_state(self, move_3):
        ...
        assert towers[0].size() < 3 or towers[1].size() > 0 or towers[2].size() > 0
...
class TestMoveSolvesCorrectly:
    ...
    def test_total_moves_equals_2n_minus_1(self, tmp_step_file):
        ...
        writer = StepWriter(step_file=tmp_step_file, echo_to_stdout=False)
        m = Move(towers=towers, num_discs=num_discs, step_by_step=False, step_writer=writer)
        count = 0
        while m.next():
            count += 1
...
class TestMoveWriteStep:
    def test_write_step_creates_file_content(self, tmp_step_file):
        ...
        writer = StepWriter(step_file=tmp_step_file, echo_to_stdout=False)
        m = Move(towers=towers, num_discs=2, step_by_step=False, step_writer=writer)
        m.next()
        writer.close()
