...
This file is the **shared doctrine** for the system:

- content standards that must hold regardless of who does the work
- renderer and artifact contracts that must hold regardless of implementation
- geometry, hierarchy, and presentation invariants that every review should enforce
...
- The `init()` pattern should be:
  ...
  3. Ensure the inline fallback stays synchronized with the external JSON files that drive normal rendering.
...
- If a directed Layer 1 spine remains visible in a deeper-layer review, prefer
  **direct visible-node vis-network edges** for that spine first. Do not insert
  extra plain root spokes just because deeper hierarchy is present.
- If you introduce helper nodes as `vis-network` edge ports for routing control,
  treat them as a last-mile geometry tool only:
  - they must not become visible as detached edge endpoints
  - they must not make arrows appear to stop short of the actual visible node
  - if a helper-port route looks less connected than a direct visible-node edge,
    revert to the simpler direct visible-node edge
...
1. a geometry-aware validation step by the renderer or the implementation owner
...
If validation fails, the layout is not acceptable yet. Any implementation-specific
workflow for renderer validation, browser inspection, and approval should live
...
- If the map is in a deeper-layer review, this rule still holds:
  - keep the single black directional spine
  - do not add extra plain black root→branch spokes alongside it
  - keep hierarchy edges plain and branch-owned
...
Node fills that sit above visible edges should usually be **opaque**. If a
node fill is semi-transparent, `vis-network` edges drawn behind the node can
show through the body and create the false impression that the edge runs into
the node center instead of terminating at the perimeter. Prefer de-emphasis
through:
...
Do not rely on translucent node fills when clean edge attachment matters.
...
- **Semi-transparent node fills** can make correctly attached vis-network edges
  look like they run through the node interior. If edge attachment looks wrong,
  check fill opacity before changing the routing model.
- **Helper-node ports that are mathematically near the boundary but visually short
  of it** make vis-network edges look disconnected. If this happens, use direct
  visible-node edges or recompute the port more faithfully.
